...
RSpec.describe RgGen::Core::OutputBase::SourceFileComponentFactory do
  ...
  it 'ドキュメント用ではないコンポーネントのみを生成する' do
    root_component = factory.create(configuration, root)
    expect(root_component.register_blocks.size).to eq 1
    expect(root_component.registers.size).to eq 1
  end
end
